...
if menu == "📊 Total Customers Successfully acquired & Target Achievement":
    ...
    if uploaded_file_transaction:
        df_new_customers = df_transaction[df_transaction["Unique_Code"].isin(df_unique["Unique_Code"])]
...
# ====================== MENU: JUMLAH TOTAL CASE HARIAN ====================== #
    ...
    if uploaded_file_input:
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.lineplot(x=df_input["Date"], y=df_input["Members"], marker="o", linestyle="-", color="blue", ax=ax)
...
# ====================== MENU: JUMLAH TOTAL PENJUALAN HARIAN ====================== #
    ...
    if uploaded_file_input:
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.lineplot(x=df_input["Date"], y=df_input["Amount"], marker="o", linestyle="-", color="green", ax=ax)
...
# ====================== MENU: JUMLAH RETENTION MEMBER BARU ====================== #
    ...
    if uploaded_file_transaction:
        ...
        # Hitung jumlah Customer dan Total Amount per Retention Days
        retention_data = df_transaction.groupby("Unique_Code").agg({"Date": "nunique", "Amount": "sum"})
